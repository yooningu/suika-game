...
const boundaryPadding = 5;  // ✅ 새 변수 선언
const {
...
const Game = {
	...
	const minX = wallPad + boundaryPadding;
	const maxX = Game.width - wallPad - boundaryPadding;
	Game.elements.previewBall.position.x = Math.min(Math.max(e.mouse.position.x, minX), maxX);  // ✅ 여기도 적용
});
...
addFruit: function (x) {
	...
	const previewClampedX = Math.min(Math.max(render.mouse.position.x, minX), maxX);  // ✅ 마우스 따라가는 것도 제한
	Game.elements.previewBall = Game.generateFruitBody(previewClampedX, previewBallHeight, Game.currentFruitSize, {
		isStatic: true,
		collisionFilter: { mask: 0x0040 }
	});
